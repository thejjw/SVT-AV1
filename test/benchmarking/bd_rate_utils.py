...
"""
Utility functions for BD-rate analysis on image coding results.

This module provides functions to:
1. Compute BD-rates per image against an anchor encoder
2. Calculate average BD-rates across images
3. Plot BD-rate vs speed/preset graphs

Usage examples:
    # Basic usage with default settings
    python bd_rate_utils.py

    # Custom anchor and quality metric
    python bd_rate_utils.py --anchor svtav1_enc --anchor-speed 7 --metric psnr_y

    # Different input/output paths
    python bd_rate_utils.py --input per_image_results.csv --output-dir results/
"""
...
def compute_average_bd_rates(
    ...
    bd_rate_stats = (
        plot_data.groupby(["encoder", "speed"])
        .agg(
            {
    ...
    try:
        encoding_time_stats = (
            per_image_df.groupby(["encoder", "speed"])
            .agg({"encoding_time": ["mean", "std", "count"]})
            .round(4)
        )

        # Flatten column names
        encoding_time_stats.columns = [
            "avg_encoding_time",
            "std_encoding_time",
            "num_samples",
        ]
        encoding_time_stats = encoding_time_stats.reset_index()

        # Merge BD-rate and encoding time statistics
        avg_results = pd.merge(
            bd_rate_stats, encoding_time_stats, on=["encoder", "speed"], how="left"
        )
    except Exception as e:
        print(
...
def main():
    """
    Main function to run the complete BD-rate analysis pipeline.
    """
    parser = argparse.ArgumentParser(
        description="Compute BD-rates and create plots from image coding results",
    ...
    parser.add_argument(
        "--input",
        "-i",
        default="/data/repos/fbsource/per_image_results.csv",
        help="Path to per_image_results.csv file",
    )
    ...
    parser.add_argument("--anchor", default="jpegli_enc", help="Anchor encoder name")

    parser.add_argument(
        "--anchor-speed", type=int, default=3, help="Anchor encoder speed"
    )

    parser.add_argument(
        "--metric",
        default="vmaf",
        choices=["vmaf", "psnr_y", "ssim", "ms_ssim", "SSIMULACRA2"],
        help="Quality metric to use for BD-rate computation",
    )
    ...
    # File paths
    bd_rates_csv = os.path.join(args.output_dir, "bd_rates_per_image.csv")
    avg_bd_rates_csv = os.path.join(args.output_dir, "average_bd_rates.csv")
    plot_path = os.path.join(args.output_dir, "average_bd_rates_plot.png")

    # Step 1: Compute BD-rates per image
    bd_rate_df = compute_bd_rates_per_image(
        per_image_csv_path=args.input,
        anchor_encoder=args.anchor,
        anchor_speed=args.anchor_speed,
        output_csv_path=bd_rates_csv,
        quality_metric=args.metric,
    )

    # Step 2: Compute average BD-rates
    ...
    if not avg_bd_rate_df.empty:
        plot_average_bd_rates(
            avg_bd_rate_df=avg_bd_rate_df,
            output_path=plot_path,
            title=f"Average BD-rate vs Speed/Preset (vs {args.anchor} speed {args.anchor_speed})\nMetric: {args.metric}",
