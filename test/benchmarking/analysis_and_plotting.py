...
def _create_single_rd_plot(
    image_data: pd.DataFrame, image: str, metric: str
) -> plt.Figure:
    ...
    for (encoder, speed), group in image_data.groupby(["encoder", "speed"]):
        if len(group) > 1:  # Only plot if we have multiple points
            ...
            # Add quality annotations on points
            for _, row in group_sorted.iterrows():
                ax.annotate(
                    f"q{int(row['quality'])}",
                    (row["file_size_mb"], row[metric]),
                    xytext=(5, 5),
            ...
            for dataset in datasets:
                ...
                for image in dataset_images:
                    for metric in available_metrics:
                        ...
                        pbar.set_description(f"Processing {dataset}/{metric}/{image}")
                        pbar.update(1)
...
def _create_unified_csv_files(
    ...
    return summary_avg_bd_rates_df
...
def run_bd_rate_analysis(
    ...
    # Create unified BD-rate CSV files
    summary_bd_rates_df: pd.DataFrame | None = _create_unified_csv_files(
