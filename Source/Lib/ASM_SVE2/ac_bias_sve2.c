...
static int psy_energy_8x8_sve2(const uint8_t* src, ptrdiff_t src_stride) {
    int32x4_t dc;
    int32x4_t sum;
    hadamard_8x8_sve2(src, src_stride, NULL, 0, NULL, &sum, &dc);
...
static int32x4_t psy_energy_16x16_sve2(const uint8_t* src, ptrdiff_t src_stride) {
    int32x4_t dc_coeff[4];
    int32x4_t sum[4];

    hadamard_8x8_sve2(src + 0 * src_stride + 0, src_stride, NULL, 0, NULL, &sum[0], &dc_coeff[0]);
    hadamard_8x8_sve2(src + 0 * src_stride + 8, src_stride, NULL, 0, NULL, &sum[1], &dc_coeff[1]);
    hadamard_8x8_sve2(src + 8 * src_stride + 0, src_stride, NULL, 0, NULL, &sum[2], &dc_coeff[2]);
    hadamard_8x8_sve2(src + 8 * src_stride + 8, src_stride, NULL, 0, NULL, &sum[3], &dc_coeff[3]);
