...
uint64_t svt_search_one_dual_neon(int *lev0, int *lev1, int nb_strengths, uint64_t **mse[2], int sb_count, int start_gi,
    ...
    uint64_t tot_mse[64 * ((end_gi + 3) & ~3)];
    size_t   nb_num    = nb_strengths;
    size_t   start_idx = start_gi;
    size_t   end_idx   = end_gi;
    size_t   i         = 0;

    if (sb_count >= 2) {
        const uint64_t *mse_0_0_ptr = mse[0][0];
        const uint64_t *mse_0_1_ptr = mse[0][1];
        const uint64_t *mse_1_0_ptr = mse[1][0];
        const uint64_t *mse_1_1_ptr = mse[1][1];

        uint64x2_t best_mse0 = vdupq_n_u64(find_best_mse(mse_0_0_ptr, mse_1_0_ptr, lev0, lev1, nb_num));
        uint64x2_t best_mse1 = vdupq_n_u64(find_best_mse(mse_0_1_ptr, mse_1_1_ptr, lev0, lev1, nb_num));

        size_t j = start_idx;
        do {
            uint64_t  *tot_mse_ptr = tot_mse + j * 64;
            ...
            do {
                ...
                vst1q_u64(tot_mse_ptr + k + 0, vaddq_u64(best1_0, best0_0));
                vst1q_u64(tot_mse_ptr + k + 2, vaddq_u64(best1_1, best0_1));
    ...
    if (i < (size_t)sb_count) {
        const uint64_t *mse_0_ptr = mse[0][i];
        const uint64_t *mse_1_ptr = mse[1][i];

        uint64x2_t best_mse0 = vdupq_n_u64(find_best_mse(mse_0_ptr, mse_1_ptr, lev0, lev1, nb_num));
    ...
    do {
        ...
        do {
            ...
            if (tot_mse[j * 64 + k] < best_tot_mse) {
                best_tot_mse = tot_mse[j * 64 + k];
    ...
    lev0[nb_num] = best_id0; // Add the identified luma strength to the list of selected luma strengths
    lev1[nb_num] = best_id1; // Add the identified chroma strength to the list of selected chroma strengths
    return best_tot_mse;
}
