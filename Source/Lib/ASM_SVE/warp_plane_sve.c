...
static AOM_FORCE_INLINE int16x8_t horizontal_filter_4x1_f1_6tap_beta0(const uint8x16_t in, const int8x16_t filter,
                                                                      const uint8x16_t perm_tbl) {
    ...
    uint16x8_t res = vcombine_u16(vqrshrun_n_s32(sum, ROUND0_BITS), vdup_n_u16(0));

    return vreinterpretq_s16_u16(res);
}

static AOM_FORCE_INLINE int16x8_t horizontal_filter_4x1_f1_8tap_beta0(const uint8x16_t in, const int8x16_t filter,
                                                                      const int32x4_t f0, const uint8x16_t perm_tbl,
                                                                      const uint8x16_t tbl_idx0_3) {
    const int32x4_t add_const = vdupq_n_s32(1 << (8 + FILTER_BITS - 1));
    ...
    const int32x4_t samples_0_3 = vreinterpretq_s32_u8(vqtbl1q_u8(in, tbl_idx0_3));
...
static AOM_FORCE_INLINE int16x8_t horizontal_filter_8x1_f1_6tap_beta0(const uint8x16_t in, const int8x16_t filter,
    ...
    uint8x16_t perm_samples[2] = {vqtbl1q_u8(in, perm_tbl.val[0]), vqtbl1q_u8(in, perm_tbl.val[1])};
...
static AOM_FORCE_INLINE int16x8_t horizontal_filter_8x1_f1_8tap_beta0(const uint8x16_t in, const int8x16_t filter,
                                                                      const int16x4_t f0, const uint8x16x2_t perm_tbl) {
    const int32x4_t add_const = vdupq_n_s32(1 << (8 + FILTER_BITS - 1));
...
static AOM_FORCE_INLINE void warp_affine_horizontal_sve(const uint8_t *ref, int width, int height, int stride,
    ...
    if (p_width == 4) {
        if (beta == 0) {
            if (alpha == 0) {
                ...
                if ((f_ptr[0] | f_ptr[1]) == 0) {
                    uint8x16_t perm_tbl = vld1q_u8(svt_kMatMul6PermuteTbl);
                    ...
                    APPLY_HORIZONTAL_SHIFT(horizontal_filter_4x1_f1_6tap_beta0, filter, perm_tbl);
                    ...
                    const int8x16_t filter = vcombine_s8(x_filter, vext_s8(x_filter, x_filter, 7));
                    APPLY_HORIZONTAL_SHIFT(horizontal_filter_4x1_f1_6tap_beta0, filter, perm_tbl);
                } else {
                    const uint8x16_t perm_tbl   = vld1q_u8(svt_kMatMul8PermuteTbl);
                    const uint8x16_t tbl_idx0_3 = vld1q_u8(svt_kTblIdx0_3);

                    // Stagger filter for use with the matrix multiply
                    // instructions.
                    // { f1, f2, f3, f4, f5, f6, f7, 0, 0, f1, f2, f3, f4, f5, f6, f7 }
                    const int8x16_t filter = vcombine_s8(vext_s8(x_filter, vdup_n_s8(0), 1),
                                                         vset_lane_s8(0, x_filter, 0));
                    const int32x4_t f0     = vdupq_n_s32(f_ptr[0]);

                    APPLY_HORIZONTAL_SHIFT(horizontal_filter_4x1_f1_8tap_beta0, filter, f0, perm_tbl, tbl_idx0_3);
                }
            } else {
        ...
        if (beta == 0) {
            if (alpha == 0) {
                ...
                const int8x8_t x_filter = vmovn_s16(f_s16);
                if ((f_ptr[0] | f_ptr[1]) == 0) {
                    uint8x16x2_t perm_tbl = vld1q_u8_x2(svt_kMatMul6PermuteTbl);
                    // Offset the permutation table to match filter layout.
                    ...
                    const int8x16_t filter = vcombine_s8(vext_s8(x_filter, x_filter, 2),
                                                         vext_s8(x_filter, x_filter, 1));
                    APPLY_HORIZONTAL_SHIFT(horizontal_filter_8x1_f1_6tap_beta0, filter, perm_tbl);
                } else if ((f_ptr[0] | f_ptr[7]) == 0) {
                    uint8x16x2_t perm_tbl = vld1q_u8_x2(svt_kMatMul6PermuteTbl);
                    // Offset the permutation table to match filter layout.
                    ...
                    uint8x16x2_t perm_tbl = vld1q_u8_x2(svt_kMatMul8PermuteTbl);
                    // Stagger filter for use with the matrix multiply instructions.
                    // { f1, f2, f3, f4, f5, f6, f7, 0, 0, f1, f2, f3, f4, f5, f6, f7 }
                    const int8x16_t filter = vcombine_s8(vext_s8(x_filter, vdup_n_s8(0), 1),
                                                         vset_lane_s8(0, x_filter, 0));
                    ...
                    APPLY_HORIZONTAL_SHIFT(horizontal_filter_8x1_f1_8tap_beta0, filter, f0, perm_tbl);
                }
            } else {
...
void svt_av1_warp_affine_sve(const int32_t *mat, const uint8_t *ref, int width, int height, int stride, uint8_t *pred,
    ...
    for (int i = 0; i < p_height; i += 8) {
        for (int j = 0; j < p_width; j += 8) {
            ...
            int16x8_t tmp[15];
            warp_affine_horizontal_sve(ref, width, height, stride, p_width, p_height, alpha, beta, x4, y4, i, tmp);
